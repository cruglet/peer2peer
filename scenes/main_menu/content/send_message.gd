...
func add_message(user_id: int, message: String):
	...
	msg.message_time = Time.get_time_string_from_system()
	msg.message = message
	...
	UserData.store_message(current_user, message, user.user_id)
	P2P.broadcast(P2P.MESSAGE, message.to_utf8_buffer())
